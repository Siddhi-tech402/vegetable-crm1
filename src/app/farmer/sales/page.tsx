...
export default function FarmerSalesPage() {
  ...
  const loadSales = useCallback(async () => {
    ...
    try {
      ...
      (allPayments as any[]).forEach(payment => {
         payment.settlements?.forEach((settlement: any) => {
             if (settlement.billNumber) {
                 const prev = paymentMap.get(settlement.billNumber) || 0;
                 paymentMap.set(settlement.billNumber, prev + (settlement.amount || 0));
             }
         });
      });
      ...
      const items: SaleItem[] = (allBills as any[]).map((bill: any) => {
        const buyers = (bill.buyers || []).map((b: any) => ({
          ...
          amount: b.subtotal || b.grossAmount || b.netAmount || 0,
        }));
        const totalAmount = bill.totalSaleAmount || bill.totals?.grossAmount || buyers.reduce((s: number, b: any) => s + b.amount, 0);
        const commission = bill.totalCommission || bill.totals?.totalCommission || 0;
        const marketFee = bill.totalMarketFee || bill.totals?.totalMarketFee || 0;
        const hamali = bill.totalHamali || bill.totals?.totalHamali || 0;
        const totalDeductions = bill.totalDeductions || bill.totals?.totalDeductions || commission + marketFee + hamali;
        const netPayable = bill.netPayableToFarmer || bill.totals?.netPayable || totalAmount - totalDeductions;
        ...
        const calculatedPaid = paymentMap.get(bill.billNumber) || bill.paidAmount || 0;
        let finalStatus = bill.status || 'pending';
        if (calculatedPaid >= netPayable && netPayable > 0) finalStatus = 'settled';
        else if (calculatedPaid > 0 && finalStatus !== 'settled') finalStatus = 'partial';
    ...
    },
    {
      ...
      render: (value: number) => (
        <span className="text-red-600 dark:text-red-400">
          {Math.abs(value) > 0 ? `₹${Math.abs(value).toLocaleString()}` : '-'}
        </span>
      ),
    },
    {
  ...
  // Calculate totals
  const totalSales = filteredSalesData.reduce((sum, item) => sum + item.totalAmount, 0);
  const totalDeductions = filteredSalesData.reduce((sum, item) => sum + item.totalDeductions, 0);
  const totalNetPayable = filteredSalesData.reduce((sum, item) => sum + item.netPayable, 0);
  const totalPaid = filteredSalesData.reduce((sum, item) => sum + item.paidAmount, 0);
          ...
          <div className="space-y-4">
            ...
            <div className="border-t border-gray-200 dark:border-gray-700 pt-4 space-y-2">
              ...
              <div className="flex justify-between text-red-600">
                ...
                <span>₹{Math.abs(selectedBill.commission).toLocaleString()}</span>
              </div>
              <div className="flex justify-between text-red-600">
                <span>Market Fee</span>
                <span>₹{Math.abs(selectedBill.marketFee).toLocaleString()}</span>
              </div>
              <div className="flex justify-between text-red-600">
                <span>Hamali</span>
                <span>₹{Math.abs(selectedBill.hamali).toLocaleString()}</span>
              </div>
